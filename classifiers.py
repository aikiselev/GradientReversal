...
class GRL_classifier():
    ...
    def build(self):
        ...
        # создаем слой обращения градиента
...
class ClassicClassifier():  # Same interface as GRL_Classifier
    def __init__(self, _feature_extractor, _label_classifier, _domain_classifier, _lam):
        self.feature_extractor = _feature_extractor
        self.label_classifier = _label_classifier
        self.model = None
        self.build()
        self.classifier_type = 'classic'
    ...
    def build(self):
        ...
        self.model = Model(input=[model_input], output=label_class)
        self.model.compile(loss=['categorical_crossentropy'], metrics=['accuracy'], optimizer='sgd')

    def fit(self, x, y, domain_prediction, validation_split=0., epoch_count=75,
            batch_size=64, verbose=0, callbacks=[]):
        self.model.fit([x], y, nb_epoch=epoch_count, batch_size=batch_size,
                       validation_split=validation_split, verbose=verbose, callbacks=callbacks)
    ...
    def predict_probs(self, x):
        return self.model.predict([x], batch_size=256, verbose=0)[:, 1]
